...
/// 1. 该类不负责末端六维外力解释；
/// 2. 该类不负责扭矩传感器硬件通道映射�?
/// 3. 该类输出�?motorEstimatedTorque_current 是基于电机参数的估计值，不等同于外界扰动力矩�?
...
public:
	...
	/// @brief 更新关节测量力矩�?	/// @param jointMeasuredTorque_sensor 按关节顺序排列的关节测量力矩�?
	HicStatus updateJointMeasuredTorque_sensor(const double* jointMeasuredTorque_sensor);

	/// @brief 更新�?coordinator 计算出的动力学模型力矩缓存�?	/// @note observer 不计算动力学，只保存最近一次状态更新对应的模型力矩快照�?
	HicStatus updateModelTorqueEstimate(const HicEstimatedDynamicsTorques& torques);

	/// @brief 便捷接口：一次性更新原始位置、电流和时间戳�?
	/// @param jointPosition 当前关节位置�?
	/// @param motorCurrent 当前电机电流�?
	/// @param currentTime 当前控制周期时间戳，单位 s�?
	/// @note 该接口会基于“当前位�?+ 上一周期缓存 + 当前时间戳”内部差分估计：
	/// 1. 关节速度�?
	/// 2. 关节加速度�?
	/// 3. 基于电流参数的电机估计力矩�?
	...
	/// @brief 输出标准化机器人状态�?	/// @param stateOut 输出滤波后的关节状态与电机估计力矩�?
	HicStatus getRobotState(HicRobotState& stateOut) const;

	/// @brief 读取滤波后的关节位置，单�?rad�?
	HicStatus getFilteredJointPosition(double* jointPosition) const;
	/// @brief 读取滤波后的关节速度，单�?rad/s�?
	HicStatus getFilteredJointVelocity(double* jointVelocity) const;
	/// @brief 读取滤波后的关节加速度，单�?rad/s^2�?
	HicStatus getFilteredJointAcceleration(double* jointAcceleration) const;
	/// @brief 读取滤波后的电机电流，单�?A�?
	HicStatus getFilteredMotorCurrent(double* motorCurrent) const;
	/// @brief 读取滤波后的关节实测力矩，单�?N.m�?
	HicStatus getFilteredJointMeasuredTorque_sensor(double* jointMeasuredTorque_sensor) const;
	/// @brief 读取由电机电流、力矩常数、减速比和传动效率估算的关节侧力矩，单位 N.m�?	/// @note coordinator 会用该值减去动力学模型力矩，得到关节外力矩估计并送入 forceObserver_�?
	HicStatus getMotorEstimatedTorque_current(double* motorEstimatedTorque_current) const;
	/// @brief 读取最近一次由 coordinator 同步进来的动力学模型力矩快照�?
	HicStatus getModelTorqueEstimate(HicEstimatedDynamicsTorques& torquesOut) const;
...
private:
	...
	bool enabled);
	HicStatus estimateMotorTorque_current();
...
private:
	...
	double rawJointMeasuredTorque_sensor_[HIC_MAX_JOINTS];
	...
	double filteredJointMeasuredTorque_sensor_[HIC_MAX_JOINTS];

	double motorEstimatedTorque_current_[HIC_MAX_JOINTS];
	HicEstimatedDynamicsTorques modelTorqueEstimate_;

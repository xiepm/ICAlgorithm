...
public:
	...
	/// @brief 更新外部实测关节力矩�?
	/// @note 该量主要进入状态观�?感知链路，本身不直接等价于零力或阻抗主输出�?
	HicStatus updateJointMeasuredTorque_sensor(const double* jointMeasuredTorque_sensor);
	...
	/// 若切换到具体模式，优先使用各�?startForceControl... 接口�?
	...
	/// @brief 启动力控模式下的关节空间阻抗模式�?	/// @note 外部 C API 通过 hic_start_force_control_mode(..., HIC_FORCE_CONTROL_MODE_JOINT_IMPEDANCE) 进入�?
	...
	HicStatus setNullspaceParameters(
	...
	/// @brief 设置关节空间阻抗配置�?	/// @note 传入�?targetPosition 已在 C API 边界转换�?rad�?
	HicStatus setJointImpedanceConfig(const HicJointImpedanceConfig& config);
	/// @brief 只更新单个关节的关节空间阻抗配置�?	/// @note jointIndex 为内�?0 起始索引；targetPosition 已在 C API 边界转换�?rad�?
	HicStatus setSingleJointImpedanceConfig(
	...
	HicStatus setJointImpedanceParameters(
	...
	/// @brief 在线更新基于电流力矩整定得到的动力学线性参数。
	HicStatus setDynamicsLinearParameters_current(const double* dynamicParams);
	/// @brief 在线更新基于扭矩传感器整定得到的动力学线性参数。
	HicStatus setDynamicsLinearParameters_sensor(const double* dynamicParams);
	/// @brief 在线更新末端负载质量和质心�?
	...
	HicStatus computeCartesianImpedanceTorqueCommand(
		double* jointTorqueCommand,
		bool* jointProtectionStatus);
	/// @brief 统一力控力矩输出入口�?	/// @note 根据当前 forceControlMode_ 分发到零力、笛卡尔阻抗或关节阻抗的力矩计算函数�?	/// 本函数只输出关节侧力矩，不进行电流换算�?
	HicStatus computeForceControlTorqueCommand(
		double* jointTorqueCommand,
		bool* jointProtectionStatus);
	/// @brief 统一力控电流输出入口�?	/// @note 包括关节阻抗在内的所有力控模式都先计算力矩，再统一转换为电流�?
	...
	/// @brief 获取最近一次状态更新时计算出的动力学模型力矩分项和外力矩估计�?	/// @note 该缓存由 updateExternalTorqueEstimateFromRobotState() �?hic_update_state_estimates() 链路中刷新�?
	HicStatus getEstimatedDynamicsTorques(HicEstimatedDynamicsTorques& torquesOut) const;
	/// @brief 读取当前基础配置快照�?
	HicStatus getConfigSnapshot(HicControlConfig& configOut) const;
	/// @brief 获取最近一次阻抗核输出的末�?wrench 命令�?
...
private:
	...
	HicStatus runCartesianImpedanceStep(
		double* jointTorqueCommand,
		bool* jointProtectionStatus);
	/// @brief 计算关节阻抗模式下的关节侧目标力矩�?	/// @note 链路为：读取 q/dq -> 可选读取外力矩估计 -> 计算阻抗力矩
	/// -> 叠加重力/科氏补偿 -> 统一安全限幅�?
...
/// @brief 将关节力矩命令换算为电机电流命令�?/// @note 换算公式依赖 torqueConstant * gearRatio * transmissionEfficiency�?/// @note 如果启用了电流限制，换算后的电流会按配置限幅�?
	...
	/// @brief 基于电机估计力矩和动力学模型刷新关节外力矩估计�?	/// @note 该函数在 updateRobotState() 成功后调用，将电流反推链路接�?forceObserver_�?
	HicStatus updateExternalTorqueEstimateFromRobotState();

	/// @brief 将最�?config_ 中与状态观测相关的字段同步回状态观测器�?
...
private:
	...
	/// @brief 最近一次状态更新时缓存的动力学模型力矩和外力矩估计�?
	HicEstimatedDynamicsTorques lastEstimatedDynamicsTorques_;
	/// @brief 零力模式的基础速度阻尼系数�?

...
void clearDynamicsTorqueTerms(HicDynamicsTorqueTerms& terms)
{
	terms.jointCount = 0;
	clearArray(terms.inertiaTorque, HIC_MAX_JOINTS);
	clearArray(terms.gravityTorque, HIC_MAX_JOINTS);
	clearArray(terms.coriolisTorque, HIC_MAX_JOINTS);
	clearArray(terms.frictionTorque, HIC_MAX_JOINTS);
	clearArray(terms.modelTorque, HIC_MAX_JOINTS);
}
...
int hic_set_dynamics_linear_parameters_current(
	RTS_IEC_INT groupId,
	const double dynamic_params[HIC_MAX_DYNAMIC_PARAMS])
{
	...
	return g_hicCoordinator->setDynamicsLinearParameters_current(dynamic_params);
}

int hic_set_dynamics_linear_parameters_sensor(
	RTS_IEC_INT groupId,
	const double dynamic_params[HIC_MAX_DYNAMIC_PARAMS])
{
	const int readyStatus = requireCoordinator(groupId);
	...
	return g_hicCoordinator->setDynamicsLinearParameters_sensor(dynamic_params);
}
...
{
	/// @brief 调试期高频状态输入接口�?	/// @note 当前主链路以 jointPosition �?motorCurrent 为输入：
	/// 1. C API �?jointPosition �?deg 转为内部 rad�?	/// 2. coordinator �?q/current/time 送入 robotStateObserver_�?	/// 3. robotStateObserver_ 估计 dq/ddq/motorEstimatedTorque_current�?	/// 4. coordinator 刷新 forceObserver_ 中的外力矩估计�?
	...
	HicControlConfig configSnapshot = {};
	
	const int configStatus = g_hicCoordinator->getConfigSnapshot(configSnapshot);
	
	std::fprintf(stderr,
			"[hic_update_state_estimates] getRobotState status=%d getConfigSnapshot status=%d synchronized state/dynamics snapshot:\n",
			stateStatus,
			configStatus);
	...
	std::fprintf(stderr,
				"  joint[%d] input_q_rad=%.6f state_q_rad=%.6f inputCurrent=%.6f A filteredCurrent=%.6f A "
				"Kt=%.6f gearRatio=%.6f efficiency=%.6f motor=%.6f gravity=%.6f coriolis=%.6f friction=%.6f model=%.6f external=%.6f\n",
				i,
				joint_position_rad[i],
				observedState.jointPosition[i],
				input->motorCurrent[i],
				observedState.motorCurrent[i],
				configSnapshot.torqueConstant[i],
				configSnapshot.gearRatio[i],
				configSnapshot.transmissionEfficiency[i],
				observedState.motorEstimatedTorque_current[i],
				observedState.gravityTorque[i],
				observedState.coriolisTorque[i],
				observedState.frictionTorque[i],
				observedState.modelTorque[i],
				observedState.externalTorque_current[i]);
		}
...
int hic_set_impedance_parameters(
	RTS_IEC_INT groupId,
	const double impedance_params[HIC_MAX_IMPEDANCE_PARAMS])
{
	...
	int index = 0;
	const bool enableExternalTorqueCompensation = (impedance_params[index++] > 0.5);

	double jointStiffness[HIC_MAX_JOINTS] = { 0.0 };
	double jointDamping[HIC_MAX_JOINTS] = { 0.0 };
	for (int i = 0; i < HIC_MAX_JOINTS; ++i)
	{
		jointStiffness[i] = impedance_params[index++];
		jointDamping[i] = impedance_params[index++];
	}

	HicImpedanceGains cartesianGains = {};
	for (int i = 0; i < HIC_CARTESIAN_DIM; ++i)
	{
		cartesianGains.stiffness[i] = impedance_params[index++];
	}
	for (int i = 0; i < HIC_CARTESIAN_DIM; ++i)
	{
		cartesianGains.damping[i] = impedance_params[index++];
	}

	double nullspaceTarget[HIC_MAX_JOINTS] = { 0.0 };
	double nullspaceStiffness[HIC_MAX_JOINTS] = { 0.0 };
	double nullspaceDamping[HIC_MAX_JOINTS] = { 0.0 };
	for (int i = 0; i < HIC_MAX_JOINTS; ++i)
	{
		nullspaceTarget[i] = impedance_params[index++];
		nullspaceStiffness[i] = impedance_params[index++];
		nullspaceDamping[i] = impedance_params[index++];
	}

	HicStatus status = g_hicCoordinator->setJointImpedanceParameters(
		jointStiffness,
		jointDamping,
		enableExternalTorqueCompensation);
	if (status != HIC_STATUS_OK)
	{
	...
	status = g_hicCoordinator->setCartesianImpedanceGains(cartesianGains);
	if (status != HIC_STATUS_OK)
	{
	...
	return g_hicCoordinator->setNullspaceParameters(
		nullspaceTarget,
		nullspaceStiffness,
		nullspaceDamping);
}
...
int hic_set_single_joint_impedance_config(
	RTS_IEC_INT groupId,
	const HicSingleJointImpedanceConfig* config)
{
	...
	if (config->jointIndex <= 0 || config->jointIndex > HIC_MAX_JOINTS)
	{
	
	return HIC_STATUS_ERROR_INVALID_PARAM;
	}

	const int internalJointIndex = config->jointIndex - 1;
	return g_hicCoordinator->setSingleJointImpedanceConfig(
		internalJointIndex,
		config->stiffness,
		config->damping,
		degToRad(config->targetPosition),
		config->targetVelocity,
		config->targetAcceleration,
		config->enableExternalTorqueCompensation,
		config->externalTorqueSource);
}
...
{
	/// @brief 力控统一电流命令出口�?	/// @note 外部周期性调用该接口获取最终下发到电机电流环的目标电流�?	/// 内部链路为：C API -> computeForceControlCurrentCommand()
	/// -> computeForceControlTorqueCommand() -> 当前力控子模式的力矩计算函数
	/// -> convertTorqueToCurrent()�?	/// @note joint_protection_status 表示对应关节在力矩或电流限幅中被保护/截断�?
...
int hic_get_force_control_torque_commands(
	...
	bool joint_protection_status[HIC_MAX_JOINTS])
{
	/// @brief 力控统一力矩命令出口�?	/// @note 外部可用该接口直接查看当前力控算法输出的关节侧目标力矩，单位 N.m�?	/// 与电流命令接口相比，本接口不会执�?torque -> current 换算�?	/// @note 在关节阻抗模式下，输出通常由阻抗力矩、重力补偿和可选科�?离心补偿组成�?	/// 并且已经经过统一安全限幅�?
	...
	if (state_out)
		{
			...
			clearArray(state_out->jointMeasuredTorque_sensor, HIC_MAX_JOINTS);
			clearArray(state_out->motorEstimatedTorque_current, HIC_MAX_JOINTS);
			clearArray(state_out->gravityTorque, HIC_MAX_JOINTS);
			clearArray(state_out->coriolisTorque, HIC_MAX_JOINTS);
			clearArray(state_out->frictionTorque, HIC_MAX_JOINTS);
			clearArray(state_out->modelTorque, HIC_MAX_JOINTS);
			clearArray(state_out->modelTorque_current, HIC_MAX_JOINTS);
			clearArray(state_out->modelTorque_sensor, HIC_MAX_JOINTS);
			clearArray(state_out->externalTorque_current, HIC_MAX_JOINTS);
			clearArray(state_out->externalTorque_sensor, HIC_MAX_JOINTS);
		}
	...
	HicEstimatedDynamicsTorques* torques_out)
{
	...
	if (torques_out)
		{
			clearDynamicsTorqueTerms(torques_out->terms);
			clearDynamicsTorqueTerms(torques_out->terms_current);
			clearDynamicsTorqueTerms(torques_out->terms_sensor);
			clearArray(torques_out->externalTorque_current, HIC_MAX_JOINTS);
			clearArray(torques_out->externalTorque_sensor, HIC_MAX_JOINTS);
		}
	...
	std::memset(torques_out, 0, sizeof(*torques_out));
	return g_hicCoordinator->getEstimatedDynamicsTorques(*torques_out);
}

...
// - hic_controller_cli 像“上层调试终�?+ 控制算法测试入口�?
//
// 新手第一次读这个文件时，可以抓住一条主线：
// “先收状�?-> �?update_state -> 再启动模�?-> 再看输出�?
...
// 统一打印布尔数组�?
//
// 在这�?demo 里，bool 数组主要用来表示“每个关节是否触发保护”�?
// 为了让输出更紧凑，这里把 true / false 打印�?1 / 0�?
//
// 例如�?
...
//
// 这三步为什么都放在这里�?
// - hic_initialize_control：相当于创建并初始化控制器内部对�?
// - hic_set_joint_torque_sensor_parameters：让控制器知道如何理解关节力矩相关参�?
...
//
// 状态发生器会不断往 socket 里发数据，如�?CLI 很久没读�?
// 缓冲区里可能已经积累了很多旧帧�?
//
// 这里采用的策略是�?
// 1. 先非阻塞地尽量把可读数据全部读出来；
...
// 为什么不直接拿第一帧就返回�?
// 因为第一帧可能已经“过时”了�?
// 假设状态发生器�?10ms 发送一次，�?CLI 100ms 才读一次，
// 那缓冲区里就可能堆了 10 帧数据。如果只取最早那一帧，
// 控制器看到的就不是“现在”的状态，而是�?00ms 前”的状态�?
//
// 所以后�?update_state 时，应该尽量使用最新状态�?
//
// 返回值：
// - true: 成功拿到一帧完整状�?
// - false: 没拿到有效数据，或者连接异�?
...
//
// 可以把这个过程理解成“喂状态”：
// - packet �?socket 收到的原始外部状�?
// - input �?HIC C API 规定的输入结构体
...
//
// 常见观察点：
// - jointPosition：当前位置是否合�?
// - jointVelocity：速度是不是和场景一�?
// - jointAcceleration：有没有被正常估算出�?
// - motorCurrent：是否接收到了外部电�?
// - motorEstimatedTorque_current：控制器内部估算的关节力�?
//
...
void printObservedState()
{
	...
	if (robotStateStatus == HIC_STATUS_OK)
	{
		...
		printVector("jointVelocity", robotState.jointVelocity, hic_test::kDemoJointCount);
			...
			printVector("motorEstimatedTorque_current", robotState.motorEstimatedTorque_current, hic_test::kDemoJointCount);
			printVector("gravityTorque", robotState.gravityTorque, hic_test::kDemoJointCount);
			printVector("coriolisTorque", robotState.coriolisTorque, hic_test::kDemoJointCount);
			printVector("frictionTorque", robotState.frictionTorque, hic_test::kDemoJointCount);
			printVector("modelTorque", robotState.modelTorque, hic_test::kDemoJointCount);
			printVector("externalTorque_current", robotState.externalTorque_current, hic_test::kDemoJointCount);
		}
...
//
// 这个函数内部会先看当�?control mode�?
// - 如果当前 forceControlMode �?ZERO_FORCE，就说明正在做零力示�?
// - 如果当前 forceControlMode 是其它力控模式，就说明正在做阻抗类控�?
// - 如果�?IDLE，就说明当前没有活动控制输出
//
...
void printHelp()
{
	std::cout
		...
		<< "                           limit = 靠近限位，适合看限位保护\n"
		<< "                           fast  = 速度较大，适合看零力进入拒绝\n"
		<< "  sync                     接收一帧最新状态，并调�?hic_update_state_estimates\n"
		<< "  zero                     �?sync，再尝试进入零力示教模式\n"
		<< "  fixed_pose               �?sync，抓取当前位姿，再进入定点位姿保持模式\n"
		<< "  fixed_pos                �?sync，抓取当前位置，再进入定点位置保持模式\n"
		<< "  fixed                    等价�?fixed_pose，保留作简短命令\n"
		<< "  step                     再同步一帧状态，并打印当前控制输出\n"
		<< "  run N                    连续执行 N �?step，例�?run 10\n"
		<< "  stop                     请求当前模式平滑退出\n"
...
int main()
{
	...
	//
	// 所以使用顺序必须是�?
	// 1. 先启�?hic_state_generator
	// 2. 再启�?hic_controller_cli
	//
	// 如果这一�?connect 失败，通常就是�?
	// - 状态发生器还没启动
	// - 端口号不一�?
	// - 本机 socket 环境有问�?
	...
	while (true)
	{
		...
		// 这些命令都依赖“最新状态”：
		// - sync: 更新控制器状�?
		// - zero/fixed: 模式启动前必须基于最新状�?
		// - step/run: 计算输出前必须先更新状�?
		// - state: 需要打印最新原始包
		//
		...
		if (command == "zero")
		{
			// 零力示教标准入口�?
			// 1. 先同步状态；
			// 2. 再调�?hic_start_force_control_mode(0, HIC_FORCE_CONTROL_MODE_ZERO_FORCE)�?
			//
		...
		if (command == "stop")
		{
			...
			//
			// 注意�?
			// - 零力模式是“准备退出”，不一�?stop 一下就立刻 idle�?
			// - 常见做法�?stop 之后�?run 几步，观察是否完成平滑退出�?
			//
			// 这和很多实时控制系统的思路一致：
		...
		if (command == "state")
		{
			...
			// 这个命令最适合排查�?
			// - 状态发生器有没有正常发数据
			// - hic_update_state_estimates 之后内部状态是否合�?
			//
			// 如果 packet 正常、但 robot state 不正常，
			// 问题大概率出�?update_state 链路或内部状态观察器�?
			if (hasPacket)
			{
		...
		if (command == "shutdown")
		{
			...
			//
			// 这个命令通常放在整套测试结束时再执行�?
			// 执行后，对端�?hic_state_generator 会收到退出请求�?

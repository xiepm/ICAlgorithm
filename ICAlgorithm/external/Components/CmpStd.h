 ...
 * </description>
 *
 * <copyright>Copyright (c) 2017-2018 CODESYS GmbH, Copyright (c) 1994-2016 3S-Smart Software Solutions GmbH. All rights reserved.</copyright>
 */
 ...
 */
#define RTS_VERSION							UINT32_C(0x03050C1E)
#define RTS_VERSION_STRING					"3.5.12.30"
#define RTS_VERSION_RC						3,5,12,30
...
typedef struct tagSMC_VECTOR3D
{
    RTS_IEC_LREAL dX;
    RTS_IEC_LREAL dY;
    RTS_IEC_LREAL dZ;
...
typedef struct tagSMC_Frame
{
    RTS_IEC_LREAL mR[8];
    //SMC_VECTOR3D vT;
    RTS_IEC_LREAL vT[4];            // in the iec, Vt is struct. But it's not work.
}SMC_Frame;

typedef struct tagAXISPOS_REF
{
	RTS_IEC_LREAL a0;
	RTS_IEC_LREAL a1;
	RTS_IEC_LREAL a2;
...
{
    RTS_IEC_BOOL xUnlimited ;                   // TRUE: no limits for this axis. dLower, dUpper have no meaning in this case.
    RTS_IEC_LREAL dLower;                       // Lower limit
    RTS_IEC_LREAL dUpper;                       // Positive limit

}AXISLIMITS;

typedef struct tagAxis_Settings
{
    AXISLIMITS limits;
